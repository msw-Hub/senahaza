          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {packages.map((pkg, index) => {
              ...
              return (
                <div
                  key={pkg.packageId}
                  className={`group relative bg-white rounded-2xl shadow-lg transition-all duration-500 overflow-hidden border ${hasCheckedItems ? "border-blue-200" : "border-gray-300 opacity-60"}`}
                  style={{
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      <div className="bg-gradient-to-br from-sky-100 via-blue-50 to-indigo-100 rounded-xl p-4 border border-sky-200 shadow-md">
                        <div className="text-sky-700 text-sm font-medium mb-1 flex items-center">
                          실제 값어치
                          <span className="text-xs text-sky-600 ml-1">({checkedItems.length}개 선택됨)</span>
                        </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                        {" "}
                        {pkg.items.map((item) => {
                          ...
                          return (
                            <div key={item.itemId} className={`flex items-center justify-between p-3 rounded-xl transition-all duration-200 group/item ${isChecked ? "bg-blue-50 border-2 border-blue-200 shadow-md" : "bg-gray-50 border border-gray-200 opacity-70"}`}>
                              ...
                              <div className="text-right">
                                <div className={`flex items-center font-semibold ${isChecked ? "text-purple-600" : "text-gray-500"}`}>
                                  ...
                                  {(currentRuby * item.quantity).toLocaleString()} 루비
                                ...
                                <div className={`text-xs ${isChecked ? "text-purple-500" : "text-gray-400"}`}>
                                  개당 {currentRuby.toLocaleString()} 루비
